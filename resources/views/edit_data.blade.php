...
<html lang="en">
  ...
  <body>
    <div class="container">
        ...
        <a class="btn btn-primary my-4" href="{{ Route('showData') }}">Show Data</a>
        
        <form action="{{route('post.update',$editData->id)}}" method="post">
            @csrf
            <div class="form-group">
                <label class="mb-2" for="">Name</label>
                <input type="name" class="form-control" value="{{ $editData->name }}" name="name" placeholder="Enter your name">
                @error('name')
